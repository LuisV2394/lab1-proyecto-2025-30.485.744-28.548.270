...
def update_block_controller(block_id):
    ...
    block.professional_id = data.get('professionalId', block.professional_id)
    block.unit_id = data.get('unitId', block.unit_id)
    block.type = data.get('type', block.type)
    block.state = data.get('state', block.state)
    block.capacity = data.get('capacity', block.capacity)
    block.notes = data.get('notes', block.notes)

    # Si se actualizan fechas/horas
    if data.get('date'):
        try:
            block.date = datetime.strptime(data.get('date'), "%Y-%m-%d").date()
        except ValueError:
            return jsonify({"error": "Fecha inválida"}), 400

    if data.get('start_time'):
        try:
            block.start_time = datetime.strptime(data.get('start_time'), "%H:%M:%S").time()
        except ValueError:
            return jsonify({"error": "Hora de inicio inválida"}), 400

    if data.get('end_time'):
        try:
            block.end_time = datetime.strptime(data.get('end_time'), "%H:%M:%S").time()
        except ValueError:
