...
Revision ID: 7b073d9b84fd
Revises: 7a1865bc4c0c
Create Date: 2026-01-04 11:12:12.350891
...
revision = '7b073d9b84fd'
down_revision = '7a1865bc4c0c'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    with op.batch_alter_table('appointment_history', schema=None) as batch_op:
        batch_op.alter_column('old_state',
               existing_type=mysql.VARCHAR(length=20),
               type_=sa.Enum('SOLICITADA', 'CONFIRMADA', 'CUMPLIDA', 'CANCELADA', 'NO_ASISTIDA', name='appointment_status_enum'),
               existing_nullable=True)
        batch_op.alter_column('new_state',
               existing_type=mysql.VARCHAR(length=20),
               type_=sa.Enum('SOLICITADA', 'CONFIRMADA', 'CUMPLIDA', 'CANCELADA', 'NO_ASISTIDA', name='appointment_status_enum'),
               existing_nullable=True)
    ...
    with op.batch_alter_table('appointments', schema=None) as batch_op:
        ...
        batch_op.alter_column('canal',
               ...
               type_=sa.Enum('PRESENCIAL', 'VIRTUAL', name='appointment_channel_enum'),
    ...
    with op.batch_alter_table('units', schema=None) as batch_op:
        batch_op.alter_column('type',
               existing_type=mysql.ENUM('SEDE', 'CONSULTORIO', 'SERVICIO'),
...
def downgrade():
    ...
    with op.batch_alter_table('units', schema=None) as batch_op:
        ...
        batch_op.alter_column('type',
               ...
               type_=mysql.ENUM('SEDE', 'CONSULTORIO', 'SERVICIO'),
               comment=None,
               existing_comment='SEDE, CONSULTORIO, SERVICIO',
               existing_nullable=False)
    ...
    with op.batch_alter_table('appointments', schema=None) as batch_op:
        batch_op.alter_column('canal',
               existing_type=sa.Enum('PRESENCIAL', 'VIRTUAL', name='appointment_channel_enum'),
               type_=mysql.VARCHAR(length=50),
               existing_nullable=False)
        batch_op.alter_column('duration_minutes',
               existing_type=mysql.INTEGER(display_width=11),
    ...
    with op.batch_alter_table('appointment_history', schema=None) as batch_op:
        ...
        batch_op.alter_column('new_state',
               existing_type=sa.Enum('SOLICITADA', 'CONFIRMADA', 'CUMPLIDA', 'CANCELADA', 'NO_ASISTIDA', name='appointment_status_enum'),
               type_=mysql.VARCHAR(length=20),
               existing_nullable=True)
        batch_op.alter_column('old_state',
               existing_type=sa.Enum('SOLICITADA', 'CONFIRMADA', 'CUMPLIDA', 'CANCELADA', 'NO_ASISTIDA', name='appointment_status_enum'),
    ...
    sa.ForeignKeyConstraint(['order_item_id'], ['order_items.id'], name='fk_result_order_item', ondelete='CASCADE'),
    ...
    sa.Column('created_at', mysql.DATETIME(), server_default=sa.text('current_timestamp()'), nullable=True),
    sa.Column('updated_at', mysql.DATETIME(), server_default=sa.text('current_timestamp() ON UPDATE current_timestamp()'), nullable=True),
    sa.ForeignKeyConstraint(['order_id'], ['orders.id'], name=op.f('fk_orderitem_order'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['result_id'], ['results.id'], name=op.f('fk_orderitem_result'), ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('id'),
    mysql_collate='utf8mb4_general_ci',
    mysql_default_charset='utf8mb4',
    mysql_engine='InnoDB'
